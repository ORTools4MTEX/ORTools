function crystalOR(job,oriP,varargin)
...
plotCrystal(job.csParent,oriP,planeP);
...
for ii = 1:length(oriC)
    ...
    plotCrystal(job.csChild,oriC(ii),planeC);
    if ~isempty(variantId)
...
plot(1.3*(p + s),'FaceColor','r') % arbitrary scaling

...
% This function defines an orientation relationship (OR) for a
% parent-child phase combination given in the *job* object as:
% - Parallel planes and directions in a GUI, or
% - Peakfitting of the parent-child boundary misorientation angle
% distribution.
%
%% Syntax:
%  job = defineORs(job)
...
if isempty(job.parentGrains)
    ...
    elseif strcmpi(loadMode,'Peak-fit') || strcmpi(loadMode,methodTypes{2})
        ...
        uiwait(f);
        [classRange,classInterval,counts] = computeP2CHistogram(job);
...
function [classRange,classInterval,counts] = computeP2CHistogram(job)
%% Function description:
...
%% Function description:
% Auxiliary function for selecting one out of several fitted ORs.
%
%% Syntax:
% [ORNumber] = setORNumber(p2c)
%

...
if numOpenFigs == 0
    ...
    currentFolder = pwd;
    [fileNameType,~,fileIdx] = uiputfile(...
        {'*.bmp','Microsoft Windows Bitmap';...
        '*.jpeg','Joint Photographic Experts Group';...
        '*.png','Portable Network Graphics';...
        '*.tiff','Tagged Image File Format';...
        '*.*','All files (*.*)'},...
        'Save file',[currentFolder,'\','*.png']);
    ...
    if fileIdx >=1 && fileIdx <= 4
        ...
        if isfile(fileNameType) || exist(fileNameType,'file') == 2
            % Find the number of files with the same name and type in the folder
            fileStruc = dir([fileName,'*',fileType]);
